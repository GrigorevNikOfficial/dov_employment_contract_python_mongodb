...
document.addEventListener('DOMContentLoaded',()=> {
    ...
    // -_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-Frontend-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-
    ...
    // Выбираем элементы по их классам
    // Переменная DOM-элемента main_menu-documents, кнопка секции "Документы"
    let main_menu_documents = document.querySelector('.main_menu-documents');
    ...
    main_menu_documents.addEventListener('click', () => {
        // Перебор всех справочников, entry - внутренняя переменная, содержащая текущий перебираемый циклом элемент
        // Удаление класса active постепенно у справочников "Физ лица", "Организации", "Товары". С удалением класса гаснет подсветка кнопки белым цветом.
        main_menu_guides_block_list.forEach((entry) => {
            entry.classList.remove("active")
        });

        // Удаление класса у контент-таблицы Физ лица, управляющего ее видимостью
        table_section_individuals.classList.remove("active");
        // Удаление класса у контент-таблицы Организации, управляющего ее видимостью
        table_section_organizations.classList.remove("active");
        // Удаление класса у контент-таблицы Товары, управляющего ее видимостью
        table_section_items.classList.remove("active");

        // Добавления класса у кнопки секции "Документы", подсвечивающего ее белым цветом
        main_menu_documents.classList.add("active");
        // Добавления класса у контент-таблицы Документы, управляющего ее видимостью
