...
async function collection_employment_contract_add() {
	...
	let emp_employer_name =
		document.getElementById("emp_employer_name").options[document.getElementById("emp_employer_name").selectedIndex]
			.textContent;
	let emp_employer_fio =
		document.getElementById("emp_employer_fio").options[document.getElementById("emp_employer_fio").selectedIndex]
			.textContent;
	...
	let emp_employee_fio =
		document.getElementById("emp_employee_fio").options[document.getElementById("emp_employee_fio").selectedIndex]
			.textContent;
	...
	let emp_supervisor_fio =
		document.getElementById("emp_supervisor_fio").options[
			document.getElementById("emp_supervisor_fio").selectedIndex
		].textContent;
	...
	if (
		await eel.collection_employment_contract_add(
			emp_num_dover,
			emp_city,
			emp_date_day,
			emp_date_month,
...
document.addEventListener("click", async function (event) {
	...
	if (event.target.classList.contains("table_row_delete") && event.target.closest(".employment_contact")) {
		...
		const num_emp = event.target.closest(".table_row").querySelector("div:nth-child(2)").textContent;
...
document.addEventListener("click", async function (event) {
	...
	if (event.target.classList.contains("table_row_change") && event.target.closest(".employment_contact")) {
		// Обновляем элемент трудового договора через значение HTML элемента
		// 1. Находим ближайший элемент с классом "table_row"
		// 2. Находим второе дитя - номер документа
		num_emp_text = event.target.closest(".table_row").querySelector("div:nth-child(2)");
		emp_emp_text = event.target.closest(".table_row").querySelector("div:nth-child(3)");
		fio_emp_text = event.target.closest(".table_row").querySelector("div:nth-child(4)");
		position_emp_text = event.target.closest(".table_row").querySelector("div:nth-child(5)");
		salary_emp_text = event.target.closest(".table_row").querySelector("div:nth-child(6)");
...
frame_emp_confirm_but.addEventListener("click", async function () {
	if (frame_emp_k === 0) {
		...
		try {
			...
			let emp_date_year = document.getElementById("emp_date_year").value;
			let emp_employer_name =
				document.getElementById("emp_employer_name").options[
					document.getElementById("emp_employer_name").selectedIndex
				].textContent;
			let emp_employer_fio =
				document.getElementById("emp_employer_fio").options[
					document.getElementById("emp_employer_fio").selectedIndex
				].textContent;
			let emp_employer_position = document.getElementById("emp_employer_position").value;
			let emp_charter = document.getElementById("emp_charter").value;
			let emp_employee_fio =
				document.getElementById("emp_employee_fio").options[
					document.getElementById("emp_employee_fio").selectedIndex
				].textContent;
			let emp_employee_position = document.getElementById("emp_employee_position").value;
			let emp_work_place = document.getElementById("emp_work_place").value;
			let emp_work_address = document.getElementById("emp_work_address").value;
...
async function selector_individuals() {
	...
	while (list_data_individuals[index] && stopper) {
		...
		if (selectedText === fullName) {
			...
			document.getElementById("emp_pass_issue_for_month").value =
				list_data_individuals[index].issued_for[0].month;
