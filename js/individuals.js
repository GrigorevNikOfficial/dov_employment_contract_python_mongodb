...
async function collection_individuals_add() {
	...
	if (
		await eel.collection_individuals_add(
			frame_indiv_surn,
			frame_indiv_name,
			frame_indiv_patr,
			frame_indiv_serial,
			...
			frame_indiv_issue,
			frame_indiv_address,
			frame_indiv_issued_for_day,
			frame_indiv_issued_for_month,
			frame_indiv_issued_for_year,
			frame_indiv_kod,
		)()
	) {
...
document.addEventListener("click", async function (event) {
	if (event.target.classList.contains("table_row_delete") && event.target.closest(".individuals")) {
		const surname = event.target.closest(".table_row").querySelector("div:nth-child(2)").textContent;
		const name = event.target.closest(".table_row").querySelector("div:nth-child(3)").textContent;
		const patronymic = event.target.closest(".table_row").querySelector("div:nth-child(4)").textContent;
		const serial = event.target.closest(".table_row").querySelector("div:nth-child(5)").textContent;
		const num = event.target.closest(".table_row").querySelector("div:nth-child(6)").textContent;
...
//Справочник Физ.лица
document.addEventListener("click", async function (event) {
	if (event.target.classList.contains("table_row_change") && event.target.closest(".individuals")) {
		current_row_indiv_surname_text = event.target.closest(".table_row").querySelector("div:nth-child(2)");
		current_row_indiv_name_text = event.target.closest(".table_row").querySelector("div:nth-child(3)");
		current_row_indiv_patronymic_text = event.target.closest(".table_row").querySelector("div:nth-child(4)");
		current_row_indiv_serial_text = event.target.closest(".table_row").querySelector("div:nth-child(5)");
		current_row_indiv_num_text = event.target.closest(".table_row").querySelector("div:nth-child(6)");
		current_row_indiv_issue_text = event.target.closest(".table_row").querySelector("div:nth-child(7)");

		let record = JSON.parse(JSON.stringify(await eel.collection_individuals_load_records()()));
		let index = 0;
		while (record[index]) {
			if (
				record[index].surname === current_row_indiv_surname_text.textContent &&
