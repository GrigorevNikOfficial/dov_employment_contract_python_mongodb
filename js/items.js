...
document.addEventListener("click", async function (event) {
	if (event.target.classList.contains("table_row_delete") && event.target.closest(".items")) {
		const items_name = event.target.closest(".table_row").querySelector("div:nth-child(2)").textContent;
...
document.addEventListener("click", async function (event) {
	if (event.target.classList.contains("table_row_change") && event.target.closest(".items")) {
		current_row_items_name_text = event.target.closest(".table_row").querySelector("div:nth-child(2)");
...
frame_items_confirm_but.addEventListener("click", async function () {
	if (frame_items_k === 0) {
		...
		await eel.collection_items_update(current_row_items_name_text.textContent, current_row_items_name.value)();
