...
document.addEventListener("click", async function (event) {
	...
	if (event.target.classList.contains("table_row_delete") && event.target.closest(".organizations")) {
		...
		const organizations_name = event.target.closest(".table_row").querySelector("div:nth-child(2)").textContent;
		// Получаем название организации через значение HTML элемента
		// 1. Находим ближайший элемент с классом "table_row"
		// 2. Находим третье дитя - адрес организации
		const organizations_address = event.target.closest(".table_row").querySelector("div:nth-child(3)").textContent;
...
document.addEventListener("click", async function (event) {
	...
	if (event.target.classList.contains("table_row_change") && event.target.closest(".organizations")) {
		// Обновляем элемент организации через значение HTML элемента
		// 1. Находим ближайший элемент с классом "table_row"
		// 2. Находим второе дитя - название организации
		current_row_org_name_text = event.target.closest(".table_row").querySelector("div:nth-child(2)");
		// Обновляем элемент организации через значение HTML элемента
		// 1. Находим ближайший элемент с классом "table_row"
		// 2. Находим третье дитя - адрес организации
		current_row_org_address_text = event.target.closest(".table_row").querySelector("div:nth-child(3)");
...
let section_organizations_addBut = document.querySelector(".section.organizations " + ".section_table_add");
