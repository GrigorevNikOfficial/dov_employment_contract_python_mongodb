...
async function select_loader() {
	...
	add_select(
		document.querySelectorAll("#doc_table_row #doc_table_itemsSelect")[
			document.querySelectorAll("#doc_table_row #doc_table_itemsSelect").length - 1
		],
		list_data_items,
	);
}
...
async function selector_individuals() {
	...
	while (list_data_individuals[index] && stopper) {
		...
		const selectedText =
			document.getElementById("doc_issue_from_FIO").options[
				document.getElementById("doc_issue_from_FIO").selectedIndex
			].textContent;
		// Полное имя
...
async function collection_documents_add() {
	...
	let doc_home_org =
		document.getElementById("doc_home_org").options[document.getElementById("doc_home_org").selectedIndex]
			.textContent;
	...
	let doc_consumer_org =
		document.getElementById("doc_consumer_org").options[document.getElementById("doc_consumer_org").selectedIndex]
			.textContent;
	// Переменная, содержащая значение поля наименование плательщика из формы
	let doc_payer_org =
		document.getElementById("doc_payer_org").options[document.getElementById("doc_payer_org").selectedIndex]
			.textContent;
	...
	let doc_issue_from_FIO =
		document.getElementById("doc_issue_from_FIO").options[
			document.getElementById("doc_issue_from_FIO").selectedIndex
		].textContent;
	// Переменная, содержащая значение поля наименование получателя из формы
	let doc_issue_get_from_org =
		document.getElementById("doc_issue_get_from_org").options[
			document.getElementById("doc_issue_get_from_org").selectedIndex
		].textContent;
...
document.addEventListener("click", async function (event) {
	...
	if (event.target.classList.contains("table_row_delete") && event.target.closest(".documents")) {
		...
		const num_doc = event.target.closest(".table_row").querySelector("div:nth-child(2)").textContent;
...
document.addEventListener("click", async function (event) {
	...
	if (event.target.classList.contains("table_row_change") && event.target.closest(".documents")) {
		...
		num_doc_text = event.target.closest(".table_row").querySelector("div:nth-child(2)");
		date_with_text = event.target.closest(".table_row").querySelector("div:nth-child(3) div:nth-child(1)");
		date_for_text = event.target.closest(".table_row").querySelector("div:nth-child(3) div:nth-child(2)");
		...
		for (let obj of objs) {
			if (obj.num_dover === num_doc_text.textContent) {
				...
				while (object[index + 1]) {
					table.insertAdjacentHTML(
						"beforeend",
						'<tr id="doc_table_row"><td><input type="text"></td><td><select id="doc_table_itemsSelect"></select></td><td><input type="text"></td></tr>',
					);
					add_select(
						document.querySelectorAll("#doc_table_row #doc_table_itemsSelect")[
							document.querySelectorAll("#doc_table_row #doc_table_itemsSelect").length - 1
...
frame_doc_confirm_but.addEventListener("click", async function () {
	if (frame_doc_k === 0) {
		...
		let doc_consumer_org_new =
			document.getElementById("doc_consumer_org").options[
				document.getElementById("doc_consumer_org").selectedIndex
			].textContent;
		let doc_payer_org_new =
			document.getElementById("doc_payer_org").options[document.getElementById("doc_payer_org").selectedIndex]
				.textContent;
		let doc_issue_from_position_new = document.getElementById("doc_issue_from_position").value;
		let doc_issue_from_FIO_new =
			document.getElementById("doc_issue_from_FIO").options[
				document.getElementById("doc_issue_from_FIO").selectedIndex
			].textContent;
		let doc_get_from_org_new =
			document.getElementById("doc_issue_get_from_org").options[
				document.getElementById("doc_issue_get_from_org").selectedIndex
			].textContent;
		...
		date_with_text.textContent =
			doc_issue_with_day_new + "." + doc_issue_with_month_new + "." + doc_issue_with_year_new;
		date_for_text.textContent =
			doc_issue_for_day_new + "." + doc_issue_for_month_new + "." + doc_issue_for_year_new;
	}
	frame_doc.style.display = "none";
});
